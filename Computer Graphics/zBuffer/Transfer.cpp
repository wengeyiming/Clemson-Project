...
void Transfer::Scale(CVertex* vertex, float sx, float sy, float sz, int size) {
    ...
    for(int i = 0; i < size; i++) {
        float matrix[4] = {vertex[i].x, vertex[i].y, vertex[i].z, 1};
...
void Transfer::Perspective(CVertex *vertex, CVertex cameraPoint, float zDepth, int size, bool isPerspect) {
    if(isPerspect) {
        ...
        for(int i = 0; i < size; i++) {
            ...
            vertex[i].x = matrix[0]*zDepth/matrix[2];
            vertex[i].y = matrix[1]*zDepth/matrix[2];
            vertex[i].z = matrix[2];
        }
    } else {
        float c_matrix[MTM_SIZE][MTM_SIZE]
        = { { 1, 0, 0, -cameraPoint.x},
            ...
            { 0, 0, 0, 1} };
        for(int i = 0; i < size; i++) {
            vertex[i].x = vertex[i].x*vertex[i].z/zDepth;
            vertex[i].y = vertex[i].y*vertex[i].z/zDepth;
